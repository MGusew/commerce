...
App_list = ["quiz", "commerce"]
app_path = os.path.join(os.path.expanduser("~"),".sobotify","projects")
...
def get_projects(selected_curr_app) :
	projects=[]

	curr_app_project_path = os.path.join(app_path, selected_curr_app)

	project_files = os.listdir(curr_app_project_path)
...
def get_project_file_name(selected_curr_app, project_name):
	curr_app_project_path = os.path.join(app_path, selected_curr_app)
	return os.path.join(curr_app_project_path,str(project_name)+".xlsx")

def get_deleted_project_file_name(selected_curr_app, project_name):
	curr_app_project_path = os.path.join(app_path, selected_curr_app)
	return os.path.join(curr_app_project_path,"trash",str(project_name)+".xlsx")

def get_project_info(selected_curr_app, project_name) :
	data=pandas.read_excel(get_project_file_name(selected_curr_app, project_name))
...
class SobotifyAppGui(object):
	...
	def __init__(self):
		...
		self.apps = App_list
		self.selected_curr_app=StringVar()
		self.selected_curr_app.set(self.apps[0])
		...
		self.projects=get_projects(self.selected_curr_app.get())
		...
		self.app,self.language=get_project_info(self.selected_curr_app.get(), self.selected_curr_project.get())
		...
		col=5
		Label(MainWindow, text="__________________ app _________________").grid(row=1, column=col,padx=20,pady=5,columnspan=2)

		self.combobox_curr_app_sel=Combobox(MainWindow,textvariable=self.selected_curr_app)
		self.combobox_curr_app_sel.grid(sticky="W",row=2, column=col,padx=20,pady=5,columnspan=2)
		self.combobox_curr_app_sel['values']= self.apps
		self.combobox_curr_app_sel['width']=30
		self.combobox_curr_app_sel['state']='readonly'
		self.combobox_curr_app_sel.bind('<<ComboboxSelected>>', self.update1_project_list )
	...
	# for sobotify App window
	def update1_project_list(self, event):
		self.projects=get_projects(self.selected_curr_app.get())
		self.combobox_curr_project_sel['values']=self.projects
		self.combobox_curr_project_sel.set("")
	...
	def start_project(self) :
		self.app,self.language=get_project_info(self.selected_curr_app.get(), self.selected_curr_project.get())
		print(self.selected_curr_app.get())
		self.stop_project()
		...
		arguments.extend(('--project_file',get_project_file_name(self.selected_curr_app.get(),self.selected_curr_project.get())))
	...
	def edit_project(self):
		project_editor_proc=os.startfile(get_project_file_name(self.selected_curr_app.get(), self.combobox_curr_project_sel.get()))


	def create_project(self):
		src=get_project_file_name(self.selected_curr_app.get(), self.selected_base_project.get())
		dest=get_project_file_name(self.selected_curr_app.get(), self.new_project.get())
	...
	def delete_project(self):
		curr_app_project_path = os.path.join(app_path, self.selected_curr_app.get())
		src=get_project_file_name(curr_app_project_path, self.selected_delete_project.get())
		dest=get_deleted_project_file_name(self.selected_curr_app.get(), self.selected_delete_project.get())
		if os.path.isfile(dest):
	...
	def new_delete_project(self) :
		...
		self.combobox_base_project.set("")
